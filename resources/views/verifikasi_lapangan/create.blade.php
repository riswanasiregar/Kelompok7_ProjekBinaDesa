...
<div class="row">
    <div class="col-12 mb-4">
        <div class="card border-0 shadow mb-4">
            <div class="card-body">
                <form action="{{ route('admin.verifikasi_lapangan.store') }}" method="POST" enctype="multipart/form-data" id="verifikasiForm">
                    @csrf
                    ...
                    <div class="row">
                                ...
                                <select class="form-select @error('pendaftar_id') is-invalid @enderror"
                                    ...
                                    @foreach($pendaftar as $item)
                                        <option value="{{ $item->pendaftar_id }}"
                                            ...
                                            {{ $item->warga->nama ?? 'N/A' }} - {{ $item->programBantuan->nama_program ?? 'N/A' }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('pendaftar_id')
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- File Media (Multiple Upload) - PERBAIKAN: name="foto[]" -->
                            <div class="mb-4">
                                <label class="form-label fw-bold">
                                    <i class="fas fa-upload me-2"></i>Upload Bukti Verifikasi (Opsional)
                                </label>
                                <input type="file"
                                    name="foto[]" {{-- PERBAIKAN: foto[] bukan media[] --}}
                                    class="form-control @error('foto.*') is-invalid @enderror"
                                    accept=".jpg,.jpeg,.png,.gif" {{-- HAPUS .pdf --}}
                                    multiple
                                    id="fotoInput">
                                <div class="form-text">
                                    <i class="fas fa-info-circle me-1"></i>
                                    Format: JPG, PNG, GIF (Maks. 5MB per file). Bisa upload beberapa file sekaligus.
                                </div>
                                @error('foto.*')
...
document.getElementById('verifikasiForm').addEventListener('submit', function(e) {
    ...
    // JANGAN pakai e.preventDefault();
});

// Preview file yang dipilih - PERBAIKAN ID
document.getElementById('fotoInput').addEventListener('change', function(e) {
    const filePreview = document.getElementById('filePreview');
    const fileList = document.getElementById('fileList');
    const files = e.target.files;

    console.log('File dipilih:', files.length, 'file(s)');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Debug info
    console.log('=== VERIFIKASI LAPANGAN CREATE LOADED ===');
    console.log('Form ID:', document.getElementById('verifikasiForm') ? 'Ada' : 'Tidak ada');
    console.log('CSRF Token:', document.querySelector('meta[name="csrf-token"]') ? 'Ada' : 'Tidak ada');
});
</script>
