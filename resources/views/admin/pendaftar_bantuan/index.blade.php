                        ...
                        <tbody>
                            ...
                            @foreach($pendaftar as $data)
                                <tr>
                                    ...
                                    <td>{{ $data->warga->no_ktp ?? '-' }}</td>
                                    <td>{{ $data->program->nama_program ?? 'N/A' }}</td>
                                    <td>{{ $data->program->tahun ?? '-' }}</td>
                                    <td>
                                        <span class="badge rounded-pill {{ $data->status_label['class'] ?? 'bg-warning' }}">
                                            {{ $data->status_label['label'] ?? 'Menunggu' }}
                                        </span>
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        @if($mediaItems->count() > 0)
                                            @if($mediaItems->count() == 1)
                                                ...
                                                <img src="data:image/{{ $type }};base64,{{ $imgData }}"
                                                     ...
                                                     onclick="showImageModal('data:image/{{ $type }};base64,{{ $imgData }}', '{{ $data->warga->nama ?? 'Pendaftar' }}')">
                                            @else
                                                {{-- Tampilkan carousel untuk multiple images --}}
                                                <div id="carousel-{{ $data->pendaftar_id }}" class="carousel slide" style="width: 120px; height: 80px;">
                                                        ...
                                                        @foreach($mediaItems as $index => $media)
                                                            ...
                                                            <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                                                <img src="data:image/{{ $type }};base64,{{ $imgData }}"
                                                                     ...
                                                                     onclick="showImageModal('data:image/{{ $type }};base64,{{ $imgData }}', '{{ $data->warga->nama ?? 'Pendaftar' }} - Gambar {{ $index + 1 }}')">
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                    @if($mediaItems->count() > 1)
                                                        <button class="carousel-control-prev" type="button"
                                                                data-bs-target="#carousel-{{ $data->pendaftar_id }}"
                                                                data-bs-slide="prev"
